        ...
        <ul className="divide-y divide-ink/10">
          {order.items.map(i => (
            <li key={i.key} className="flex items-center gap-5 py-5">
              ...
              <div className="flex-1 text-sm"><p className="font-display text-xl">{i.name}</p><p className="text-ink/70">{[i.color, i.size && `T. ${i.size}`, `× ${i.quantity}`].filter(Boolean).join(' · ')}</p></div>
              <span className="text-sm">{formatPrice(i.price * i.quantity)}</span>
            </li>
          ))}
        </ul>
        <dl className="text-sm space-y-2 pt-5 border-t border-ink/10">
          <div className="flex justify-between"><dt className="text-ink/75">Sous-total</dt><dd>{formatPrice(order.subtotal)}</dd></div>
          ...
          {order.giftFee > 0 && <div className="flex justify-between"><dt className="text-ink/75">Emballage cadeau</dt><dd>{formatPrice(order.giftFee)}</dd></div>}
          <div className="flex justify-between"><dt className="text-ink/75">Livraison</dt><dd>{order.deliveryFee ? formatPrice(order.deliveryFee) : 'Offerte'}</dd></div>
        </dl>
        {order.giftMessage && (
