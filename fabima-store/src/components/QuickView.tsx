  ...
  return (
    <div className="fixed inset-0 z-[85] flex items-end sm:items-center justify-center sm:p-6">
      ...
      <div role="dialog" aria-modal="true" aria-label={`Aperçu : ${product.name}`}
        className="relative bg-ivory w-full max-w-4xl max-h-[92vh] overflow-y-auto grid sm:grid-cols-2 animate-fade-up shadow-luxe">
        ...
        <div className="p-7 sm:p-10 flex flex-col">
              ...
              <div className="flex flex-wrap gap-1.5">
                {product.sizes.map(s => (
                  <button key={s} onClick={() => { setSize(s); setSizeError(false); }}
                    className={`min-w-11 h-11 px-2 text-sm border transition-colors ${size === s ? 'bg-ink text-ivory border-ink' : sizeError ? 'border-wine/60' : 'border-ink/15 hover:border-ink'}`}>{s}</button>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mt-8 flex gap-2">
            ...
            <button onClick={() => toggleWishlist(product.id)} aria-label="Favoris" className="w-[52px] h-[52px] border border-ink/20 grid place-items-center hover:border-ink">
